...
\begin{frame}{What is a Managed Runtime System?}
  \centering
  An abstraction layer between a managed programming language and the system that executes it
  \vfill
  ...
  \begin{tikzpicture}[node distance=0cm]
    ...
    \node (oscover) [visible on=<1>, layer, below=of vm, fill=myyellow] {};

    \node (linux) [visible on=<2>, layer, below=of vm.south west, anchor=north west, minimum width=3.25cm] {GNU/Linux};
    \node (osx) [visible on=<2>, layer, right=of linux, minimum width=3.25cm] {OSX};
    \node (win) [visible on=<2>, layer, right=of osx, minimum width=3.25cm] {Windows};
    \node (other) [visible on=<2>, layer, right=of win, minimum width=1.2cm] {\dots};
    \node (x86) [layer, below=of linux.south west, anchor=north west, minimum width=4cm] {x86};
    \node (armv8) [layer, right=of x86, minimum width=4cm] {ARMv8};
    \node (other2) [layer, right=of armv8, minimum width=2.97cm] {\dots};

    \scoped[on background layer]{
      \node [fit=(program)(x86), inner sep=0, blur shadow] {};
    }

    \node [visible on=<2>, fit=(linux)(other), inner sep=0, draw=SkyBlue, thick] {};
    \node [visible on=<2>, text=SkyBlue, left=of linux] {OS};
    \node [fit=(x86)(other2), inner sep=0, draw=Orange, thick] {};
    \node [text=Orange, left=of x86] {HW};
